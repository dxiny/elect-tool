...
### 🔧 配置管理
*   **实现逻辑**：从 `localStorage` 迁移到了 `.env` 环境变量与运行时配置相结合。
*   **前端**：`src/pages/config/Index.vue` 展示当前配置。
*   **核心代码**：`src/config/app.ts` 负责读取配置。

### 🤖 AI 对话
*   **实现逻辑**：前端维护对话列表，通过流式接口 (Stream) 接收 AI 响应。
*   **数据流**：User Input -> `backend.ts` -> (IPC/HTTP) -> AI Service -> Stream Response。
...
### 📦 自动更新
*   **实现逻辑**：利用 Electron Builder 的自动更新机制 (配合 GitHub Releases)。
*   **状态**：基础配置已完成，需配合代码签名证书使用体验更佳。
