...
ElectTool 是一个基于 Electron + Vue 3 + Node.js 的桌面应用，集成了 GIS 服务、3D 模型、AI 助手等功能。项目采用前后端分离架构，支持本地离线模式与云端在线模式。
...
后端服务主要用于处理数据持久化、文件上传等业务，特别是当应用运行在“在线模式”时。

*   **服务入口 (`server/index.cjs`)**:
    ...
    ```
*   **数据库**: 使用 `better-sqlite3` 操作 SQLite 数据库，轻量且无需额外安装数据库服务，适合桌面应用嵌入。
...
由于没有购买 Apple 开发者证书（每年 $99），打包出的 macOS 应用未签名，用户安装时会提示“文件已损坏”或“无法验证开发者”。
...
### 待完成与后续规划
1.  **AI 深度集成**: 目前仅接入了基础对话，后续计划接入本地 LLM (如 Llama 3) 运行在 Electron 内部，实现完全离线的 AI 能力。
